...
const EditVehicleModal: React.FC<EditVehicleModalProps> = ({
  ...
  const [imageLama, setImageLama] = useState<
  ...
  useEffect(() => {
    ...
    const imageLama = async () => {
      const token = localStorage.getItem("access_token");
      const response = await axios.get(`/api/vehicles/get_id/${vehicle.id}`, {
        headers: {
          "Content-Type": "application/json",
          AccessToken: token || "",
        },
  ...
  const handleDeleteImageLama = async (id: number) => {
    const token = localStorage.getItem("access_token");
    if (!token) {
    ...
    try {
      const response = await fetch(`/api/vehicles/images/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          AccessToken: token,
      ...
      if (response.ok) {
        setImageLama(imageLama.filter((img) => img.id !== id));
  ...
  const handleUpdateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (files) {
        ...
        uploadMultipleResponse = await upload_multiple_file(
          formData,
          ...
          {imageLama.length !== 0 &&
            imageLama.map((data) => (
              <div key={data.id}>
                <img
                  alt="gambar"
                  src={`/api/file?filename=${data.imageUrl}`}
                ...
                <button
                  ...
                  onClick={() => handleDeleteImageLama(data.id)} // Now passing the correct id
                >
                  Hapus gambar
                </button>
              </div>
